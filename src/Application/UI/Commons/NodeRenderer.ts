...
class NodeRenderer {
     ...
     */
    applyRender() {
        (this.renderDispatcher[Renderables.Dom] || this.renderDispatcher[Renderables.Tag]) && this.#renderDom();
    ...
    #renderAttributes() {
        Object.keys(this.#node.attributes).forEach((attributeKey: string) => {
            this.#node.dom && this.#node.dom.setAttribute(attributeKey, this.#node.attributes[attributeKey]);
        });
    ...
    #renderListeners() {
        ...
        this.#node.listeners.forEach((listener: TEventListener) => {
            !listener.attached && this.#node.dom?.addEventListener(listener.name, listener.callback);
    ...
    #reRender() {
        if (this.#node.parent?.dom && this.#node.dom) {
            ...
            if (this.#node.content) {
                ...
                this.#node.children.forEach((child: VNode) => child.renderer.render(this.#node));
            }
